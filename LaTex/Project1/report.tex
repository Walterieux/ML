...
% TODO check degree
The regression technique denoted as \textit{Our Best Regression} consists of all useful data cleaning and processing steps listed in section \ref{ssec:data_proc}, as well as polynomial extension of degree $7$ on ridge regression.
...
\begin{table}[h]
	\begin{tabular}{ |c|c| } 
		...
		\hline
		Least Squares GD (and SGD) & $70.5\%$ - $0.29\%$ \\ 
		Least Squares & $??\%$ - $??\%$ \\ 
		Ridge Regression & $??\%$ - $??\%$ \\ 
